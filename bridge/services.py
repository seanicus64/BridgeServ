...
class ListenProtocol(protocol.Protocol):
    ...
    def handle_register(self, obj, protocol):
        ...
        #newnick, link_id = protocol.relay_register_user(nick, username, hostname, realname)
###        data = obj.copy()
###        data["nick"] = newnick
###        data["link_id"] = link_id
###        self.send_response(data)

        uid = protocol.factory
        i = 0
        newnick = nick
        while True:
            try:
                assert not protocol.factory.userdict.next_exists(newnick)
                break
            except:
                newnick = nick + "_" + str(i)
            i += 1
        nick = newnick
        if not self.factory.userdict.islegal(nick):
            nick = "temp+" + str(uid)
        link_id = protocol.factory.get_link_id()
        self.factory.userdict.append(uid, nick, username, hostname, real_name, link_id)
        protocol.relay_register_user(nick, username, hostname, realname)
        obj["nick"] = newnick
        obj["link_id"] = link_id
        self.send_response(obj)
        ## self.factory.irc_factory
#        uid = self.factory.get_uid()
#        i = 0
#        newnick = requested_nick 
#        while True:
#            try:
#                assert not self.factory.userdict.nick_exists(newnick)
#                break
#            except:
#                newnick = requested_nick + "_" + str(i)
#            i += 1
#        nick = newnick
#        if not self.factory.userdict.islegal(nick):
#            nick = "temp_" + str(uid)
#        link_id = self.factory.get_link_id()
#        self.factory.userdict.append(uid, nick, username, hostname, real_name, link_id)
...
class IRCProtocol(IRC):
    ...
    def relay_register_user(self, requested_nick, username, hostname, real_name):
...
#
    ...
    def irc_EOS(self, prefix, params):
        #TODO: shouldnt allow anything until finished synching
...
#        self.factory.list_channels()
    def irc_PRIVMSG(self, prefix, params):
        ...
        print("There is a private message")
        print(params[0])
        print(params[1])
        for p in self.factory.listen_factory.protocols:
            data = {}
            ...
            for ch in self.factory.chandict:
                print(ch.name, channel)
                if ch.name == channel:
                    channel_link_id = ch.link_id
                    print(channel_link_id)
                    break
            if not channel_link_id:
                return
            print("the channel link id is {}".format(channel_link_id))
